...
export class MapViewer {
  ...
  public init(routesGeoJsonUrl: string, stationsGeoJsonUrl: string) {
  ...
  private setupPopups() {
    ...
    this.map.on('click', 'stations-clusters', (e: any) => {
      const features = this.map.queryRenderedFeatures(e.point, { layers: ['stations-clusters'] });
      const clusterId = features[0].properties.cluster_id;
      this.map.getSource('stations').getClusterExpansionZoom(clusterId, (err: any, zoom: number) => {
        if (err) return;
        this.map.easeTo({
    ...
    // Cursor pointers
    const setCursorPointer = () => { this.map.getCanvas().style.cursor = 'pointer'; };
